"""Phase 5 Step 2 — DEAP genetic algorithm to evolve a format that beats ACF.

Seed individual = ACF_SEED. Fitness = accuracy (60%) + token efficiency (40%).
Model: llama3.1:8b via Ollama.

First run (verify):  POPULATION_SIZE=20, N_GENERATIONS=10  (~30 min)
Full run (overnight): POPULATION_SIZE=50, N_GENERATIONS=30  (~3-4 hrs)
"""
...
def main():
    ...
    # Guard against double-registration if module is reloaded
    ...
    # --- Establish ACF seed fitness (baseline for comparison) ---
    print("Evaluating ACF seed fitness...", flush=True)
    seed_fitness = evaluate(dict(schema_mod.ACF_SEED))[0]
    seed_metrics = get_last_metrics()
    print(f"ACF seed: fitness={seed_fitness:.4f} | tokens={seed_metrics['avg_tokens']} | acc={seed_metrics['avg_accuracy']:.2f}", flush=True)

    # --- Build initial population ---
    pop = [_make_individual(schema_mod.ACF_SEED)]
    for _ in range(POPULATION_SIZE - 1):
        pop.append(_make_individual(schema_mod.mutate(dict(schema_mod.ACF_SEED))))
    ...
    print(f"\nAgentClearfeed Phase 5 — Genetic Algorithm")
    ...
    for gen in range(1, N_GENERATIONS + 1):
        ...
        # Re-evaluate only individuals whose fitness was invalidated by operators
    ...
    # Example rendering of first query document with the evolved schema
    ...
    # --- Console summary ---
    print("\n" + "=" * 70)
    outcome = "BEAT ACF" if best_fitness > seed_fitness else "DID NOT BEAT ACF"
    print(f"RESULT: {outcome}")
    print(f"ACF seed fitness : {seed_fitness:.4f}  (tokens={seed_metrics['avg_tokens']}, acc={seed_metrics['avg_accuracy']:.2f})")
    ...
    # --- Save results ---
    output = {
        "baseline_acf_fitness": round(seed_fitness, 6),
        "baseline_acf_tokens": seed_metrics["avg_tokens"],
        "baseline_acf_accuracy": seed_metrics["avg_accuracy"],
    ...
    out_path = RESULTS_DIR / "ga_results.json"
